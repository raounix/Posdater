...
def update_articles(json_key, url, file_url):
    counter = 1
    data_file = open(file_url, "r+")
    ...
    for userid in accounts:
        ...
        for item in all_items:
            ...
            if hashed not in last_posts:
                ...
                print("{counter} - New Found ! - {title} : {url}\n".format(counter=str(counter), title=item.title.text,
                                                                           url=item.link.text))
...
def update_medium(type):
    ...
    print("----------------------------------------------------------------\n")
    if (type == "account"):
            update_articles("accounts", "https://medium.com/feed/{}", './db/medium.json')
    elif (type == "topic"):
        update_articles("topics", "https://medium.com/feed/tag/{}", './db/medium.json')
